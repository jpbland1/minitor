...
OnionCircuit* onion_circuits;
OnionService* onion_services;
...
static void v_send_init_circuit_intro( OnionService* service )
{
  ...
  do
  {
    ((CreateCircuitRequest*)onion_message->data)->end_relay = px_get_random_hsdir_relay( 0, NULL, NULL, NULL );;
...
circuit_rebuild:
  ...
  if ( working_circuit->target_status != CIRCUIT_RENDEZVOUS || working_circuit->relay_list.built_length < 1 )
  {
    if ( working_circuit->target_status == CIRCUIT_HSDIR_BEGIN_DIR )
    {
      if ( working_circuit->relay_list.built_length >= 2 )
      {
        working_circuit->target_relay_index++;
        working_circuit->service->hsdir_sent++;
        ...
        retry_length = 3;
        retry_end_relay = px_get_relay_by_index( working_circuit->service->target_relays[working_circuit->desc_index], working_circuit->target_relay_index );
      }
      else
      {
        retry_length = 3;
        retry_end_relay = malloc( sizeof( OnionRelay ) );
        memcpy( retry_end_relay, working_circuit->relay_list.tail->relay, sizeof( OnionRelay ) );
      }
    }
    else if ( working_circuit->target_status == CIRCUIT_RENDEZVOUS )
    {
    ...
    {
      retry_length = working_circuit->relay_list.length;
...
static void v_handle_conn_close( DlConnection* or_connection )
{
  ...
  {
    tmp_circuit = closed_circuit->next;

    if ( closed_circuit->or_connection == or_connection )
    {
      circ_id = closed_circuit->circ_id;

      v_remove_circuit_from_list( closed_circuit, &onion_circuits );

      // make a new intro point if we lost one
      if ( closed_circuit->target_status == CIRCUIT_ESTABLISH_INTRO )
      {
        v_send_init_circuit_intro(
          closed_circuit->service
        );
      }
      else if ( closed_circuit->target_status == CIRCUIT_RENDEZVOUS && closed_circuit->status != CIRCUIT_RENDEZVOUS )
      {
        end_relay = malloc( sizeof( OnionRelay ) );
        memcpy( end_relay, closed_circuit->relay_list.tail->relay, sizeof( OnionRelay ) );
        ...
        v_send_init_circuit(
          ...
          NULL,
          end_relay,
          closed_circuit->hs_crypto
        );
      }
      else if ( closed_circuit->target_status == CIRCUIT_HSDIR_BEGIN_DIR )
      {
        end_relay = malloc( sizeof( OnionRelay ) );
        memcpy( end_relay, closed_circuit->relay_list.tail->relay, sizeof( OnionRelay ) );
        ...
        v_send_init_circuit(
          ...
          CIRCUIT_HSDIR_BEGIN_DIR,
          closed_circuit->service,
          closed_circuit->desc_index,
          closed_circuit->target_relay_index,
          NULL,
          end_relay,
          NULL
        );
      ...
      d_destroy_onion_circuit( closed_circuit );

      free( closed_circuit );
    }

    closed_circuit = tmp_circuit;
  }
...
static void v_keep_circuitlist_alive()
{
  ...
  while ( working_circuit != NULL )
  {
    ...
    // MUTEX TAKE
    succ = xSemaphoreTake( working_circuit->or_connection->access_mutex, 500 / portTICK_PERIOD_MS );

    if ( succ == pdFALSE || d_send_packed_cell_and_free( working_circuit->or_connection, packed_cell ) < 0 )
    {
#ifdef DEBUG_MINITOR
    ...
    if ( succ == pdTRUE )
    {
      xSemaphoreGive( working_circuit->or_connection->access_mutex );
      // MUTEX GIVE
    }
...
static void v_init_service( OnionService* service )
{
  ...
  start_relay = px_get_random_hsdir_relay( 1, NULL, NULL, NULL );
  ...
  if ( d_mark_hsdir_relay_as_guard( start_relay->identity ) < 0 )
  {
    return;
  }
  ...
  for ( i = 0; i < 3; i++ )
  {
    ...
    do
    {
      ((CreateCircuitRequest*)onion_message->data)->end_relay = px_get_random_hsdir_relay( 0, NULL, start_relay->identity, NULL );;
...
void v_minitor_daemon( void* pv_parameters )
{
  ...
  while ( xQueueReceive( core_task_queue, &onion_message, portMAX_DELAY ) )
  {
    ...
    switch ( onion_message->type )
    {
      ...
      default:
#ifdef DEBUG_MINITOR
        ESP_LOGE( CORE_TAG, "Got an unknown onion message" );
#endif
        break;
    }
